...
const GaragesList = () => {
  ...
  const { garages, search_option, search_text, sort_type, sort_name } =
    useSelector((state) => state.providersScreenReducer);

  const { list, loading, records_per_page, page_index, count } = garages;
                      ...
                      <ul className="ltn__select-availability-table-row">
                        ...
                        <li className="table-data-7 text-primary">
                          {garage_actions?.includes("VIEW") && (
                            <strong>
                              <Link
                                to={`/admin/view_provider_services/garage/${record.Id}?tab=0`}
                              >
                                Services
                              </Link>
                            </strong>
                        ...
                        <li className="table-data-7">
                          {garage_actions?.includes("UPDATE") && (
                            <strong>
                              <Link
                                to={`/admin/edit_provider/garage/${record.Id}?tab=0`}
                              >
                                Edit
                              </Link>
                            </strong>
                        ...
                        <li className="table-data-7">
                          <strong>
                            <Link
                              to={`/claim/garage/garage_detail/${record.Id}`}
                            >
                              View
                            </Link>
                          </strong>

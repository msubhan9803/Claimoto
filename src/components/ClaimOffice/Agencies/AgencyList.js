...
const AgenciesList = () => {
  ...
  const { agencies, search_option, search_text, sort_type, sort_name } =
    useSelector((state) => state.providersScreenReducer);

  const { list, loading, records_per_page, page_index, count } = agencies;
                      ...
                      <ul className="ltn__select-availability-table-row">
                        ...
                        <li className="table-data-7 text-primary">
                          {agency_actions?.includes("VIEW") && (
                            <strong>
                              <Link
                                to={`/admin/view_provider_services/agency/${record.Id}?tab=0`}
                              >
                                Services
                              </Link>
                            </strong>
                        ...
                        <li className="table-data-7">
                          {agency_actions?.includes("UPDATE") && (
                            <strong>
                              <Link
                                to={`/admin/edit_provider/agency/${record.Id}?tab=0`}
                              >
                                Edit
                              </Link>
                            </strong>
